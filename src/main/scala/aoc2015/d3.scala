...
//first part
  ...
  def run(input: Array[String]) = {
    input
      .foldLeft((Map(Pos(0, 0) -> 1), Pos(0, 0)))({ case ((poss, pos), dir) =>
        val npos = dir match {
...
// second part
  ...
  def run2(input: Array[String]) = {
    val (santa, robo, _, _, _) =
      input.foldLeft(
        (Map(Pos(0, 0) -> 1), Map(Pos(0, 0) -> 1), Pos(0, 0), Pos(0, 0), 1)
        ...
        val (nSantaPoss, nSantaPos) =
          if (turn % 2 == 1)
            (santaPoss.updated(npos, santaPoss.getOrElse(npos, 0) + 1), npos)
          else (santaPoss, santaPos)
        val (nRoboPoss, nRoboPos) =
          if (turn % 2 == 0)
            (roboPoss.updated(npos, roboPoss.getOrElse(npos, 0) + 1), npos)
          else (roboPoss, roboPos)
